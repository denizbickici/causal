...
def condition_projection_causal(x, conditions, action_dim, class_dim):
    for t, val in conditions.items():
        if t != 'task':
            x[:, t, class_dim + action_dim:] = val.clone()
...
class Weighted_MSE_Geodesics(nn.Module):
    ...
    def forward(self, pred, targ, geo_pred, geo_targ):
        """
        :param pred: [B, T, task_dim+action_dim+observation_dim]
        :param targ: [B, T, task_dim+action_dim+observation_dim]
        :return:
        ...
        #print('geo_pred',geo_pred.shape, 'geo_targ',geo_targ.shape)
        loss_geo = F.mse_loss(geo_pred, geo_targ, reduction='none')
        loss_geo = loss_geo.mean()
        return loss_action + loss_geo


Losses = {
    'Weighted_MSE': Weighted_MSE,
    'Weighted_MSE_Geodesics': Weighted_MSE_Geodesics,
}
