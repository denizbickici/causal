...
class Trainer(nn.Module):
    def __init__(
        ...
        self.cls_net = MLP(input_dim=8*(z_dim*2), hidden_dim=128, output_dim=action_dim, num_layers=3)
    ...
    def _pool_to_steps(self, tensor, target_len=8):
        """Temporal pooling that keeps batch/crop dims intact."""
        if tensor.shape[-2] == target_len:
            return tensor
        orig_shape = tensor.shape
        flattened = tensor.reshape(-1, orig_shape[-2], orig_shape[-1])
        pooled = F.adaptive_avg_pool1d(flattened.transpose(1, 2), target_len).transpose(1, 2)
    ...
    def eval_pred(self):
        ...
        for batch in self.test_loader:
            ...
            if action_logits.dim() == 2:
                action_logits = action_logits.unsqueeze(1)
            act_feat = self._pool_to_steps(act_feat, target_len=8)  # [B, crops, 8, 1024]
            '''verb_feat = verb_feat[:,:,:8,:]
            noun_feat = noun_feat[:,:,:8,:]'''
