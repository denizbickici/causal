...
class Trainer(object):
	...
	def validate(self, args):
		...
		for batch in self.test_loader:
			with torch.no_grad():								
				motion_feat, spatial_verb_feat, spatial_noun_feat, verb_label, noun_label, action_label, act_logits = batch[0].cuda(), batch[1].cuda(), batch[2].cuda(), batch[3].cuda(), batch[4].cuda(), batch[5].cuda(), batch[6].cuda()
				#action_label = action_label.view(-1,1)
				motion_feat = motion_feat.to(torch.float)
				spatial_verb_feat = spatial_verb_feat.to(torch.float)
				spatial_noun_feat = spatial_noun_feat.to(torch.float)
				labels=action_label
				
				bs = motion_feat.shape[0]
				loss, flow_verb_rec_loss, flow_verb_sparsity_loss, flow_verb_kld_loss, flow_verb_structure_loss, flow_noun_rec_loss, flow_noun_sparsity_loss, flow_noun_kld_loss, flow_noun_structure_loss, class_loss, pred = self.model(flow_data=motion_feat, spatial_verb_data=spatial_verb_feat, spatial_noun_data=spatial_noun_feat, labels=labels, act_logits=act_logits, is_val=True)
	...
	def train(self, if_calculate_acc, args):
		...
		for batch in self.train_loader:
			...
			#print(motion_feat.dtype, spatial_verb_feat.dtype, spatial_noun_feat.dtype)
			motion_feat = motion_feat.to(torch.float)
			spatial_verb_feat = spatial_verb_feat.to(torch.float)
			spatial_noun_feat = spatial_noun_feat.to(torch.float)
			#print(motion_feat.dtype, spatial_verb_feat.dtype, spatial_noun_feat.dtype)
